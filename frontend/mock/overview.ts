...
const now = Date.now()
// [user_id, group_id, status, response_time_ms, 分钟前, error_code]
const logsSeed: Array<[number, number, 'success' | 'error' | 'cooldown', number, number, string | null]> = [
  ...
  [3499812267, 1056723489, 'cooldown', 12, 11, 'COOLDOWN_ACTIVE'],
  [1528967345, 768431025, 'success', 254, 17, null],
  [4012238765, 1012345678, 'error', 892, 23, 'IMAGE_FILE_MISSING'],
  [3345617789, 892345671, 'success', 128, 29, null],
  [2890123456, 976543218, 'success', 465, 36, null],
  [3109876543, 853210476, 'cooldown', 9, 45, 'COOLDOWN_ACTIVE'],
  [3567210988, 1029384756, 'success', 213, 52, null],
  [2746159042, 918273645, 'error', 675, 61, 'VOICE_ACTOR_HANDLER_ERROR'],
  [3301876542, 876543219, 'success', 391, 70, null],
  [4023567891, 987654321, 'success', 154, 82, null],
  [2954310876, 908172635, 'cooldown', 14, 95, 'COOLDOWN_ACTIVE'],
  [3812093456, 819273645, 'success', 502, 110, null],
  [4123876540, 1072635489, 'error', 923, 128, 'VOICE_ACTOR_HANDLER_ERROR'],
...
const recentLogs = logsSeed.map(([user_id, group_id, status, response_time_ms, minutesAgo, error_code], index) => ({
  id: 1000 + index,
  user_id,
  group_id,
  command: 'voice_actor',
